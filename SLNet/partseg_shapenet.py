...
def test_epoch(test_loader, model, epoch, num_part, num_classes, io):
    ...
    for batch_id, (points, label, target, norm_plt) in tqdm(enumerate(test_loader), total=len(test_loader), smoothing=0.9):
        ...
        points, label, target, norm_plt = points.cuda(non_blocking=True), label.squeeze(1).cuda(non_blocking=True), \
                                          target.cuda(non_blocking=True), norm_plt.cuda(non_blocking=True)
        seg_pred = model(points, norm_plt, to_categorical(label, num_classes))[0]  # b,n,50
...
if __name__ == "__main__":
    ...
    parser.add_argument('--batch_size', type=int, default=128, metavar='batch_size',     # 32>128
                        help='Size of batch)')
    parser.add_argument('--test_batch_size', type=int, default=64, metavar='batch_size',    # 32>64
                        help='Size of batch)')
    parser.add_argument('--epochs', type=int, default=350, metavar='N',   # 350>400
