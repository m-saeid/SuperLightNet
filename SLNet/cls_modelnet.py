...
class Classification(nn.Module):
        def __init__(self,
                 n=1024,
                 embed=[3,64,'mlp','no',0.3,1000.0,100.0],   # mlp, fourier, scaled_fourier, gaussian, harmonic, mlp2, relative, linear_coord, /learnable
                 res_dim_ratio=1.0,
                 bias=False,
                 use_xyz=True,
                 norm_mode="anchor",
...
def parse_args():
    ...
    parser.add_argument('--use_hybrid_pe', action='store_true',
                        help='Enable Fourier+Gaussian positional encoding')
    parser.add_argument('--pe_scales', type=int, nargs='+', default=[0,1,2,4,8,16],
                        help='Scales for Fourier features')   # K≈6 scales
    parser.add_argument('--pe_sigma', type=float, default=0.1,
                        help='σ for Gaussian RBF features')
    parser.add_argument('--use_pointmamba', action='store_true',
                        help='Enable global SSM block (PointMamba)')
    parser.add_argument('--mamba_layers', type=int, default=2,
                        help='Number of Mamba layers in SSM')  
...
def main():
    ...
    #    print("\n\nConfiguration already exists. Skipping execution.\n\n")
    #    return
    ...
    net = Classification(n=args.n,
                        ...
                        use_hybrid_pe=args.use_hybrid_pe,
    ...
    train_loader = DataLoader(ModelNet40(partition='train', num_points=args.n, use_hybrid_pe=args.use_hybrid_pe, pe_scales=args.pe_scales, pe_sigma=args.pe_sigma), num_workers=args.workers,
                            batch_size=args.batch_size, shuffle=True, drop_last=True)
    test_loader = DataLoader(ModelNet40(partition='test', num_points=args.n, use_hybrid_pe=args.use_hybrid_pe, pe_scales=args.pe_scales, pe_sigma=args.pe_sigma), num_workers=args.workers,
