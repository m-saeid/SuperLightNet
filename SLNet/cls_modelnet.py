...
# ************************************* GlobalContextssm ************************************
from encoder.spectral_frustum_adapter import SpectralFrustumAdapter
# ************************************* GlobalContextssm ************************************
...
class Classification(nn.Module):
    def __init__(self,
                 ...
                 classifier_mode='mlp_very_small',    
                 ):
        ...
        # ************************************* GlobalContextssm ************************************
        self.adapter = SpectralFrustumAdapter(
            dim=last_dim,
            bottleneck=32,
            curve_bits=10,
            dropout=0.1
    ...
    def forward(self, xyz, feature):
        x_list, f_list = self.encoder(xyz=xyz, x=None, feature=feature)
        feats = f_list[-1]                   # [B, C, N]
        # transpose for adapter: [B, N, C]
        feats_t = feats.permute(0, 2, 1).contiguous()
        # apply adapter
        feats_t = self.adapter(feats_t, x_list[-1]) # [B, N, C]
        # back to [B, C, N]
        feats = feats_t.permute(0, 2, 1).contiguous()
        # global pooling
        x = F.adaptive_max_pool1d(feats, 1).squeeze(-1)  # [B, C]
...
def parse_args():
    ...
    parser.add_argument('--embed', type=str, default='mlp', help='mlp')
...
def main():
    ...
    train_loader = DataLoader(ModelNet40(partition='train', num_points=args.n, out_dim=args.embed[0]), num_workers=args.workers,
                              batch_size=args.batch_size, shuffle=True, drop_last=True)
    test_loader = DataLoader(ModelNet40(partition='test', num_points=args.n, out_dim=args.embed[0]), num_workers=args.workers,
...
def train(net, trainloader, optimizer, criterion, device, ema_net=None):
    # At the top of your training script, before you start training:
