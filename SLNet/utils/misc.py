...
def save_model(net, epoch, path, acc, is_best, fold=99, **kwargs):
    state = {
        'net': net.state_dict(),
        'epoch': epoch,
        'acc': acc
    ...
    for key, value in kwargs.items():
        state[key] = value
    if fold != 99:
        filepath = os.path.join(path, f"last_checkpoint_{fold}.pth")
        torch.save(state, filepath)
        if is_best:
            shutil.copyfile(filepath, os.path.join(path, f'best_checkpoint_{fold}.pth'))
    else:
