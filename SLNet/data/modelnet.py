...
class ModelNet40(Dataset):
    def __init__(self, num_points, partition='train', in_d=3, out_d=16):
    ...
    def __getitem__(self, item):
        ...
        if self.partition == 'train':
            ...
            np.random.shuffle(pointcloud)   # 1024,3
...
if __name__ == '__main__':
    ...
    for batch_idx, (data, feature, label) in enumerate(train_loader):
        print(f"batch_idx: {batch_idx}  | data shape: {data.shape} | feature shape: {feature.shape} | ;lable shape: {label.shape}")
        break
