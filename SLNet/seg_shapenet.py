...
def _init_():
    ...
    if not os.path.exists(f'checkpoints/{args.model}/' + args.msg):
        os.makedirs(f'checkpoints/{args.model}/' + args.msg)
...
def train(args, io):
    ...
    '''Resume or not'''
    if args.resume:
        state_dict = torch.load("checkpoints/%s/best_insiou_model.pth" % args.msg,
        ...
        print(torch.load("checkpoints/%s/best_insiou_model.pth" % args.msg).keys())
    else:
    ...
    for epoch in range(args.epochs):
        ...
        if test_metrics['accuracy'] > best_acc:
            ...
            torch.save(state, 'checkpoints/seg_shapenet/%s/best_acc_model.pth' % args.msg)
        ...
        if test_metrics['shape_avg_iou'] > best_instance_iou:
            ...
            torch.save(state, 'checkpoints/seg_shapenet/%s/best_insiou_model.pth' % args.msg)
        ...
        if avg_class_iou > best_class_iou:
            ...
            torch.save(state, 'checkpoints/seg_shapenet/%s/best_clsiou_model.pth' % args.msg)
    ...
    torch.save(state, 'checkpoints/seg_shapenet/%s/model_ep%d.pth' % (args.msg, args.epochs))
...
def test(args, io):
    ...
    from collections import OrderedDict
    state_dict = torch.load("checkpoints/%s/best_%s_model.pth" % (args.msg, args.model_type),
                            map_location=torch.device('cpu'))['model']
...
if __name__ == "__main__":
    ...
    # Encoder
    args.embed = [6, args.embed_dim, args.embed]
    ...
    if not args.eval:
        io = IOStream(f'checkpoints/{args.model}/' + args.msg + '/%s_train.log' % (args.msg))
    else:
        io = IOStream(f'checkpoints/{args.model}/' + args.msg + '/%s_test.log' % (args.msg))
    io.cprint(str(args))
