...
class Encoder(nn.Module):
    def __init__(self,
        ...
        if use_hybrid_pe:
            # NEW
            #in_ch_pe = 3 + (2 * 3 * len(pe_scales) if use_hybrid_pe else 0) \
            #            + (16 if use_hybrid_pe else 0)
            in_ch_pe = 3 + (2 * 3 * len(pe_scales)) + 16
            # self.conv1 = nn.Conv1d(in_ch_pe, hidden, 1, bias=bias)
            self.embedding = mlp(in_ch_pe, embed[1])
        else:
