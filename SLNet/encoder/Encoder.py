...
class Encoder(nn.Module):
    def __init__(self,
                 n=1024,
                 embed=[3,16,'no',True],   # mlp, fourier, scaled_fourier, gaussian, harmonic, mlp2, relative, linear_coord, /learnable
                 res_dim_ratio=1.0,
                 bias=True,
                 use_xyz=True,
                 norm_mode="anchor",
                 ...
                 block1_mode = ['mlp', 'mlp', 'mlp', 'mlp'], # mlp, fourier, scaled_fourier, gaussian, harmonic, mlp2, relative, linear_coord, /learnable
        ...
        for i in range(self.stages):
            ...
            self.normalization_list.append(Normalization(d=last_ch, norm_mode=norm_mode, std_mode=std_mode, use_xyz=use_xyz))
    ...
    def forward(self, xyz, x=None, feature=None):            # [B, C, N] (2,3,1024)  feature: B,N,D
        if x is None:           # cls
            f = self.embedding(feature.permute(0,2,1))  
        else:                   # seg
            f = self.embedding(x)
...
encoder = Encoder(n=1024, embed=[3,16,'mlp',False], res_dim_ratio=0.25, bias=False, use_xyz=True, norm_mode="anchor", std_mode="BN1D",
