...
from data.scanobject_pointhop import ScanObjectNN_pointhop, pointhop_feature
...
class Classification(nn.Module):
        def __init__(self,
            ...
            self.encoder = Encoder(n=n, embed=embed, res_dim_ratio=res_dim_ratio, bias=bias, use_xyz=use_xyz,
                                   norm_mode=norm_mode, dim_ratio=dim_ratio, num_blocks1=num_blocks1,
                                   num_blocks2=num_blocks2, k_neighbors=k_neighbors,
                                   sampling_mode=sampling_mode, sampling_ratio=sampling_ratio)
            
            last_dim = embed[1]
            for d in dim_ratio:
                 last_dim *= d
            
            self.classifier = Classifier(last_dim, 15)
...
def main():
    ...
    args.embed = [3, args.embed_dim, args.embed]
    ...
    if args.seed is not None:
        ...
        torch.cuda.manual_seed(args.seed) if torch.cuda.is_available() else torch.manual_seed(args.seed)
    ...
    criterion = cal_loss
    net = net.to(device)
    printf(f'number of params: {trainable_params(net)}')
    ...
    if args.embed[-1] in ['mlp', 'tpe', 'gpe']:
        ...
        train_loader = DataLoader(ScanObjectNN_pointhop(partition='training', num_points=args.n, feature=feat_train), num_workers=args.workers,
                                batch_size=args.batch_size, shuffle=True, drop_last=True)
        test_loader = DataLoader(ScanObjectNN_pointhop(partition='test', num_points=args.n, feature=feat_valid), num_workers=args.workers,
                                batch_size=args.batch_size, shuffle=True, drop_last=False)
    else:
...
if False:#__name__ == '__main__':
    ...
    feature = None
    print("===> testing Model ...")
    for embd in [[3,16,'mlp'],[3,18,'tpe'],[3,16,'gpe'],[3,16,'pointhop']]:
        print(f"\n\nembd: {embd};")
