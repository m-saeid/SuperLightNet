...
INITIAL_DIM=3
EMBED_DIM=16
RES_DIM_RATIO=(0.25 0.5 0.75)
BIAS=False
USE_XYZ=True
NORM_MODE=('center' 'anchor')

DIM_RATIO=('2-2' '1-2' '2-1')

NUM_BLOCKS1=('2-2' '1-2' '2-1')
TRANSFER_MODE='mlp-mlp'
BLOCK1_MODE='mlp-mlp'

NUM_BLOCKS2=('2-2' '1-2' '2-1')
BLOCK2_MODE='mlp-mlp'

K=('16-16' '24-24' '28-28' '32-32' '40-40' '64-64')
SAMPLING_MODE=('fps-fps')
SAMPLING_RATIO=('2-2')
        ...
        for k in "${K[@]}"; do
            ...
            for dim_ratio in "${DIM_RATIO[@]}"; do
                for num_blocks1 in "${NUM_BLOCKS1[@]}"; do
                    for num_blocks2 in "${NUM_BLOCKS2[@]}"; do

                        python cls_modelnet.py --n "$N" --embed "$EMBED" --initial_dim "$INITIAL_DIM" --embed_dim "$EMBED_DIM" \
                        --res_dim_ratio "$res_dim_ratio" --bias "$BIAS" --use_xyz "$USE_XYZ" --norm_mode "$norm_mode" \
