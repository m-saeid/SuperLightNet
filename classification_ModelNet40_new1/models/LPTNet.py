...
#import sys
#sys.path.append('/home/zeus/Desktop/saeid_1080/PointMLP_APES/pointnet2_ops_lib/')
...
class N2PAttention(nn.Module):
    def __init__(self, dim):
        ...
        self.ff = nn.Sequential(nn.Conv1d(dim, 4*dim, 1, bias=False), nn.LeakyReLU(0.2), nn.Conv1d(4*dim, dim, 1, bias=False))
        self.bn1 = nn.BatchNorm1d(dim)
        self.bn2 = nn.BatchNorm1d(dim)

    def forward(self, x):
        neighbors = ops.group_(x, self.K, self.group_type)  # (B, C, N) -> (B, C, N, K)
...
class Model(nn.Module):
    def __init__(self, points=1024, class_num=40, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        self.n2p_att_list = nn.ModuleList()
        ...
        for i in range(len(pre_blocks)):
            ...
            ######################################## APES ########################################
            # append n2p_att_list
            n2p_att_block = N2PAttention(dim=out_channel)
            self.n2p_att_list.append(n2p_att_block)
            ######################################## APES ########################################
    ...
    def forward(self, x):
        ...
        for i in range(self.stages):
            ...
            x_att = self.n2p_att_list[i](x) # [b,d,g]   APES
            x = self.pos_blocks_list[i](x)  # [b,d,g]
            x = x + x_att                  # [b,d,g]   APES
...
def pointMLPMini(num_classes=40, **kwargs) -> Model:
    return Model(points=1024, class_num=num_classes, embed_dim=16, groups=1, res_expansion=0.25,
                   activation="relu", bias=False, use_xyz=True, normalize="anchor",                    # use_xyz=True > False
                   dim_expansion=[2, 2, 2, 1], pre_blocks=[1, 1, 2, 1], pos_blocks=[1, 1, 2, 1],
                   k_neighbors=[24,24,24,24], reducers=[2, 2, 2, 2], **kwargs)

def pointMLPMini18(num_classes=40, **kwargs) -> Model:
    return Model(points=1024, class_num=num_classes, embed_dim=18, groups=1, res_expansion=0.25,
                   activation="relu", bias=False, use_xyz=True, normalize="anchor",                    # use_xyz=True > False
                   dim_expansion=[2, 2, 2, 1], pre_blocks=[1, 1, 2, 1], pos_blocks=[1, 1, 2, 1],
                   k_neighbors=[24,24,24,24], reducers=[2, 2, 2, 2], **kwargs)

def pointMLPMini20(num_classes=40, **kwargs) -> Model:
    return Model(points=1024, class_num=num_classes, embed_dim=20, groups=1, res_expansion=0.25,
                   activation="relu", bias=False, use_xyz=True, normalize="anchor",                    # use_xyz=True > False
                   dim_expansion=[2, 2, 2, 1], pre_blocks=[1, 1, 2, 1], pos_blocks=[1, 1, 2, 1],
                   k_neighbors=[24,24,24,24], reducers=[2, 2, 2, 2], **kwargs)

def pointMLPMini22(num_classes=40, **kwargs) -> Model:
    return Model(points=1024, class_num=num_classes, embed_dim=22, groups=1, res_expansion=0.25,
                   activation="relu", bias=False, use_xyz=True, normalize="anchor",                    # use_xyz=True > False
                   dim_expansion=[2, 2, 2, 1], pre_blocks=[1, 1, 2, 1], pos_blocks=[1, 1, 2, 1],
                   k_neighbors=[24,24,24,24], reducers=[2, 2, 2, 2], **kwargs)

def pointMLPMini24(num_classes=40, **kwargs) -> Model:
    return Model(points=1024, class_num=num_classes, embed_dim=24, groups=1, res_expansion=0.25,
                   activation="relu", bias=False, use_xyz=True, normalize="anchor",                    # use_xyz=True > False
                   dim_expansion=[2, 2, 2, 1], pre_blocks=[1, 1, 2, 1], pos_blocks=[1, 1, 2, 1],
                   k_neighbors=[24,24,24,24], reducers=[2, 2, 2, 2], **kwargs)

def pointMLPMini26(num_classes=40, **kwargs) -> Model:
    return Model(points=1024, class_num=num_classes, embed_dim=26, groups=1, res_expansion=0.25,
                   activation="relu", bias=False, use_xyz=True, normalize="anchor",                    # use_xyz=True > False
                   dim_expansion=[2, 2, 2, 1], pre_blocks=[1, 1, 2, 1], pos_blocks=[1, 1, 2, 1],
                   k_neighbors=[24,24,24,24], reducers=[2, 2, 2, 2], **kwargs)

def pointMLPMini28(num_classes=40, **kwargs) -> Model:
    return Model(points=1024, class_num=num_classes, embed_dim=28, groups=1, res_expansion=0.25,
                   activation="relu", bias=False, use_xyz=True, normalize="anchor",                    # use_xyz=True > False
                   dim_expansion=[2, 2, 2, 1], pre_blocks=[1, 1, 2, 1], pos_blocks=[1, 1, 2, 1],
                   k_neighbors=[24,24,24,24], reducers=[2, 2, 2, 2], **kwargs)

def pointMLPMini30(num_classes=40, **kwargs) -> Model:
    return Model(points=1024, class_num=num_classes, embed_dim=30, groups=1, res_expansion=0.25,
                   activation="relu", bias=False, use_xyz=True, normalize="anchor",                    # use_xyz=True > False
                   dim_expansion=[2, 2, 2, 1], pre_blocks=[1, 1, 2, 1], pos_blocks=[1, 1, 2, 1],
                   k_neighbors=[24,24,24,24], reducers=[2, 2, 2, 2], **kwargs)
...
if __name__ == '__main__':
    ...
    #model = pointMLP()
    model = pointMLPMini()
    print(f'number of params APES_global: {trainable_params(model)}')
    out = model(data)
    print(out.shape)
