...
sys.path.append("E:\Papers\SLNet\code\SLNet\encoder")
...
class Encoder(nn.Module):
    def __init__(self, n=1024,
        ...
        self.embedding = Embedding(embed[0], embed[1], mode=embed[2])
        ...
        for i in range(self.stages):
            ...
            self.normalization_list.append(Normalization(last_ch, norm_mode))

            self.blocks1_list.append(Block1(last_ch, out_ch, num_blocks1[i], res_dim_ratio=res_dim_ratio,
                                             bias=bias, use_xyz=use_xyz))
            
            self.agg_list.append(Agg("adaptive_max_pool1d"))

            self.blocks2_list.append(Block2(out_ch, num_blocks2[i], res_dim_ratio=res_dim_ratio, bias=bias))

            last_ch = out_ch


    def forward(self, xyz, x=None):            # [B, C, N] (2,3,1024)
        
        f = self.embedding(xyz) if x is None else self.embedding(x)        # [B, D, N] (2,16,1024)   MLP(3>16)
...
encoder = Encoder(n=1024, embed=[3,16,'mlp'], res_dim_ratio=0.25, bias=False, use_xyz=True, norm_mode="anchor",
